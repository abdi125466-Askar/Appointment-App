...
/** ✅ slugify: "Birth Certificate Application" -> "birth_certificate_application" */
const slugify = (str = "") =>
  String(str)
    .toLowerCase()
    .trim()
...
export default function PublicAppointmentPage() {
  ...
  useEffect(() => {
    ...
    // OPTIONAL mapping short slugs -> service code
    const slugToCode = {
      passport: "PP-001",
      national_id: "ID-001",
      birth_certificate: "BC-001",
  ...
  /**
   * ✅ REQUIRE FILE:
   * - Submit won't activate unless file exists
   */
  const canSubmit = useMemo(() => {
    return (
      fullName.trim().length >= 3 &&
  ...
  const onSubmit = async () => {
    // ✅ show file error if missing
    if (!file) {
      setFileError("Fadlan soo geli PDF file-ka (Required) ka hor intaadan Submit.");
      return;
    }
    ...
    // ✅ multer field must match backend: uploadPdf.single("file")
    formData.append("file", file);
    ...
    if (createPublicAppointment.fulfilled.match(resAction)) {
      ...
      // ✅ Save silently (Track page auto-fill)
      if (trackingId) localStorage.setItem(TRACKING_KEY, String(trackingId));
          ...
          <div className="p-6 md:p-8">
            ...
            <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (v && v < todayStr) {
                        setDateError(
                          "Ma dooran kartid taariikh hore. Fadlan dooro maanta ama kadib."
                        );
                        setAppointmentDate("");
                        return;
                      }
              ...
              <div className="md:col-span-2">
                ...
                {fileError && (
                  <div className="mt-2 text-sm font-bold text-red-700 flex items-center gap-2">
                    <AlertCircle size={16} />
                    {fileError}
                  </div>
                )}
            ...
            <div className="mt-6 flex flex-col md:flex-row gap-3 items-start md:items-center">
              ...
              {!file && (
                <p className="text-xs font-bold text-slate-600">
                  ⚠️ Submit will activate after you upload a PDF file.
                </p>
              )}
            </div>
