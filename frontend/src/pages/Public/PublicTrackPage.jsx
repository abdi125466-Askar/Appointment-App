...
// ✅ Path note: PublicTrackPage.jsx wuxuu ku jiraa pages/Public, stepper-kuna pages/Public/components
...
export default function PublicTrackPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      else setErrorMsg(res?.message || "Appointment not found.");
    } catch (err) {
      setErrorMsg(err?.response?.data?.message || "Appointment not found.");
    } finally {
      setLoading(false);
    }
  };

  const prettyDate = (d) => (d ? new Date(d).toDateString() : "—");
      ...
      <div className="w-full px-6 lg:px-16 xl:px-24 py-16">
        ...
        {data && !errorMsg && (
          <div className="max-w-6xl mx-auto bg-white shadow-2xl border border-blue-100 rounded-3xl p-10">
            ...
            {/* ✅ STEPPER */}
            <AppointmentStepper status={data?.status} />
          </div>
        )}
      </div>
    </section>
