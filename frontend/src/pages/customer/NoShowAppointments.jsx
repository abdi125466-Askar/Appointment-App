...
export default function NoShowAppointments() {
  const dispatch = useDispatch();
  const noShow = useSelector((s) => s.appointments.byStatus.NO_SHOW || []);
  const loading = useSelector((s) => s.appointments.loading);
  const error = useSelector((s) => s.appointments.error);

  useEffect(() => {
    dispatch(fetchAppointmentsByStatus("NO_SHOW"));
  }, [dispatch]);
    ...
    <div className="p-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-200 flex items-center justify-between">
          <p className="font-bold text-slate-700">Total: {noShow.length}</p>
