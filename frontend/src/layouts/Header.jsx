...
export default function Header({
  ...
  // ✅ FIX: users page route is /dashboard/users (not /users)
  const isUsersPage =
  ...
  // debounce URL update (prevents too many updates)
  useEffect(() => {
    if (!isUsersPage) return;

    const t = setTimeout(() => {
      const next = searchTerm.trim();
      // ✅ FIX: update URL under /dashboard/users
      const url = next
        ? `/dashboard/users?search=${encodeURIComponent(next)}`
        : `/dashboard/users`;
